...
```
docker run -d -p 8888:8888 choptiu/all-spark-notebook
```
...
1. Configure each slave with [the `--no-switch_user` flag](https://open.mesosphere.com/reference/mesos-slave/) or create the `choptiu` user on every slave node.
